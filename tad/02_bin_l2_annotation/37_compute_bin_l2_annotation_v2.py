...
# -*- coding: utf-8 -*-
"""
37_compute_bin_l2_annotation_v2.py
=====================================
処理内容:
  ...
  - group_specificity (Diff_specific = n=1, Diff_shared = n>=2, Static = diff=0)
    を割り当て
  - sensitivity 用に group_sensitivity_* 列を追加
      S1 (primary): Diff_specific_n1 / Diff_shared_n2plus / Static
      S2: Diff_specific_n1or2 / Diff_shared_n3plus / Static
      S3: Diff_specific_n1to3 / Diff_shared_n4plus / Static
      S4: Diff_specific_n1 / (skip n=2) / Diff_shared_n3plus / Static
      S5: Diff_specific_n1 / Diff_shared_n5plus / Static (core-shared)
  - 論文 v200 Fig.3a (10 L2 classes) と Fig.3d (specific vs shared) 用の
...
入力:
  ...
  --outdir   : 出力ディレクトリ
出力:
  bin_l2_annotation_v2.tsv.gz (bin-level annotation, 1 row per bin)
  bin_l2_annotation_summary_v2.tsv (QC カウント)
入力/出力パス:
  入力  : F_01_BOUNDARY_MASTER (common.paths_v1)
          LINEAGE_XLSX         (common.paths_v1)
  出力先: OUT_02_BIN_L2_ANNOT
  v2 の変更点:
    - argparse は維持しつつ全引数に common/paths_v1.py 由来の default を設定
    - 引数なし `python 37_compute_bin_l2_annotation_v2.py` で実行可能
    - 入力 master を v8 → v9 に変更
    - 出力 suffix を v1 → v2
"""
...
def assign_group_columns(anno: pd.DataFrame) -> pd.DataFrame:
    """Assign primary and sensitivity group labels."""
    ...
    def lbl_s2(di: int, ni: int) -> str:
        # S2: specific = n<=2, shared = n>=3
        if di == 0:
            return "Static"
        if ni <= 2:
            return "Diff_specific_n1or2"
        return "Diff_shared_n3plus"

    def lbl_s3(di: int, ni: int) -> str:
        # S3: specific = n<=3, shared = n>=4
        if di == 0:
            return "Static"
        if ni <= 3:
            return "Diff_specific_n1to3"
        return "Diff_shared_n4plus"

    def lbl_s4(di: int, ni: int) -> str:
        # S4: specific = n=1, shared = n>=3 (n=2 は除外)
        if di == 0:
            return "Static"
        if ni == 1:
            return "Diff_specific_n1"
        if ni == 2:
            return "Diff_excluded_n2"
        return "Diff_shared_n3plus"

    def lbl_s5(di: int, ni: int) -> str:
        # S5: specific = n=1, shared = n>=5 (core-shared)
        if di == 0:
            return "Static"
        if ni == 1:
            return "Diff_specific_n1"
        if ni >= 5:
            return "Diff_shared_n5plus"
        return "Diff_middle_n2to4"

    anno["group_primary"] = [lbl_primary(di, ni) for di, ni in zip(diff, n)]
    anno["group_s2"]      = [lbl_s2(di, ni)      for di, ni in zip(diff, n)]
    anno["group_s3"]      = [lbl_s3(di, ni)      for di, ni in zip(diff, n)]
    anno["group_s4"]      = [lbl_s4(di, ni)      for di, ni in zip(diff, n)]
    anno["group_s5"]      = [lbl_s5(di, ni)      for di, ni in zip(diff, n)]
    return anno
...
def make_summary(anno: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["group_primary", "group_s2", "group_s3", "group_s4", "group_s5"]:
...
def main() -> None:
    ...
    # Phase F (2026-04-21): parse_args() を実際に使う。default は paths_v1 由来なので
    # CLI 引数なしで実行した場合の挙動は従来どおり。override も有効化。
    args = parse_args()
    ...
    log("=" * 60)
    log("Start 37_compute_bin_l2_annotation_v2.py")
