#!/usr/bin/env python3
# 42_compute_mssng_exon_exclusion_v2.py
#
# 処理内容:
...
#    reduction) + BH/Holm 多重検定補正を追加した v2。
#  - Input:
...
#      7. 出力 TSV:
#           mssng_exon_exclusion_v2.tsv (全 7 行)
#           mssng_exon_exclusion_summary_v2.tsv (4 discovery-positive 行)
#  - 出力先: /lustre12/home/kushima-pg/tad04212026/13_exon_exclusion_mssng/output_v2/
#  - 処理時間を先頭と末尾で記録
#
...
#SBATCH --output=42_compute_mssng_exon_exclusion_v2_%j.out
#SBATCH --error=42_compute_mssng_exon_exclusion_v2_%j.err
...
OUT_DIR = BASE_TAD / "13_exon_exclusion_mssng" / "output_v2"
OUT_TSV = OUT_DIR / "mssng_exon_exclusion_v2.tsv"
OUT_SUMMARY = OUT_DIR / "mssng_exon_exclusion_summary_v2.tsv"
...
def fit_gee_mssng(
    ...
    try:
        ...
        for xc in x_cols:
            if X_df[xc].std() == 0:
                return [{
                    ...
                    "p_two": np.nan,
                    "n_case": int(y.sum()), "n_ctrl": int(len(y) - y.sum()),
                    "carrier_case": 0, "carrier_ctrl": 0,
                    "n_clusters": len(np.unique(groups)),
                    "fit_status": f"zero_variance_{xc}",
                } for xc in x_cols]
        ...
        for xc in x_cols:
            ...
            carr_case = int(df.loc[df[y_col] == 1, xc].gt(0).sum())
            carr_ctrl = int(df.loc[df[y_col] == 0, xc].gt(0).sum())
            out.append({
                "exposure": xc,
                "beta": beta, "se": se,
                ...
                "carrier_case": carr_case,
                "carrier_ctrl": carr_ctrl,
                "n_clusters": len(np.unique(groups)),
        ...
        out = [{
            ...
            "carrier_case": 0, "carrier_ctrl": 0,
            "n_clusters": 0,
            "fit_status": f"gee_failed:{str(e)[:60]}",
        } for xc in x_cols]
    return out
...
def fit_firth_mssng(
    ...
    try:
        ...
        out = [{
            ...
            "n_case": 0, "n_ctrl": 0,
            "carrier_case": 0, "carrier_ctrl": 0,
            "fit_status": f"firth_failed:{str(e)[:60]}",
        } for xc in x_cols]
    return out
...
def main() -> None:
    ...
    log("=" * 72)
    log("Script 13 v2 START: MSSNG exon-exclusion sensitivity (tad04212026)")
    log("  42_compute_mssng_exon_exclusion_v2.py")
    log("=" * 72)
    ...
    log("=" * 72)
    log(
        f"Script 13 v2 DONE: 42_compute_mssng_exon_exclusion_v2.py  "
