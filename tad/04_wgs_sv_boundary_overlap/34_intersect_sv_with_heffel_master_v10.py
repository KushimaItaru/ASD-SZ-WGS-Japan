#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ファイル名: 34_intersect_sv_with_heffel_master_v10.py
# - 処理内容:
...
def main():
    t0 = time.time()
    ensure_output_dirs()
    # v10: output_v10 ディレクトリを確実に作成
    OUT_04_V10.mkdir(parents=True, exist_ok=True)
    ...
    parser = argparse.ArgumentParser(
        description="Intersect rare DEL/DUP events with Heffel boundary master v9 and build "
                    ...
                    "2026-04-22 update (v10): adds sample-level top-1% CNV burden QC."
    )
    ...
    parser.add_argument("--cnv-sample-counts", default=str(CNV_SAMPLE_COUNTS),
                        help="Per-sample CNV count file used to define top-percentile "
                             "high-burden samples (matches Script 03 v8 input).")
    parser.add_argument("--high-burden-percentile", type=float, default=0.99,
                        help="Percentile threshold for sample-level high-burden exclusion "
                             "(default 0.99; matches Script 03 v8 event-level QC).")
    # v10: デフォルト outdir を output_v10 に変更 (paths_v1 は不変)
    parser.add_argument("--outdir", default=str(OUT_04_V10))
    ...
    log("Start 34_intersect_sv_with_heffel_master_v10.py")
    ...
    # v10: 出力 suffix を v10 に
    overlap_path = outdir / "sample_boundary_event_overlap_v10.tsv.gz"
    ...
    # v10: 出力 suffix を v10 に
    burden_path = outdir / "sample_mechanistic_burden_v10.tsv"
    ...
    # v10: 出力 suffix を v10 に
    summary_path = outdir / "sample_mechanistic_burden_summary_v10.tsv"
