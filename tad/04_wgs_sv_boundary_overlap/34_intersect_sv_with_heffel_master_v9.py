#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ファイル名: 34_intersect_sv_with_heffel_master_v9.py
# - 処理内容:
#   - rare SV events と Heffel boundary master v9 を overlap し、event-level overlap table を作成する
#   - 2026-04-21 update (in-place, ChatGPT review blockers 2 & 3):
#       Blocker 2: protein-coding gene overlap カウント (total_gene_DEL/DUP, log1p_total_gene_DEL/DUP)
#                  を sample_mechanistic_burden_v9.tsv にマージする。旧 v18 pipeline の run_step3()
#                  相当ロジックを本体に統合。Step 6 R B' regression は log1p_total_gene_{DEL|DUP}
#                  を hard-require するため Step 4 で供給する。
#       Blocker 3: bin overlap filter を `overlap_bp >= 1` から
#                  `overlap_frac_boundary >= 0.10` に変更 (旧 v18 pipeline の bin_overlap_thr 相当)。
#                  --min-overlap-frac-boundary で上書き可（default=0.10）。
#                  `--min-overlap-bp` は後方互換のために残し、両者を AND で適用する。
...
# ---------------------------------------------------------------------------
# 3 系統 burden 集計 (v8 で導入、現行 v9 でも踏襲)
...
def aggregate_sample_burden_by_type(
    ...
    """
    3 系統の burden を SV type × sample 単位で集計する (v8 で導入、現行 v9 でも踏襲)。
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Intersect rare DEL/DUP events with Heffel boundary master v9 and build "
                    "full-cohort sample-level burden table with matching_level stratification. "
                    "2026-04-21 update: integrates protein-coding gene covariate computation "
                    "(total_gene_DEL/DUP) and overlap_frac_boundary 0.10 threshold."
    )
    parser.add_argument("--sv", default=str(F_03_WGS_SV_EVENTS))
    ...
    parser.add_argument("--outdir", default=str(OUT_04_SV_BOUNDARY_OVERLAP))
    ...
    log("Start 34_intersect_sv_with_heffel_master_v9.py")
    ...
    for p in [sv_path, master_path, sample_info_path, pca_path, gtf_path]:
        if not p.exists():
    ...
    log(f"Full cohort samples retained: {full_sample.shape[0]}")
    ...
    overlap_path = outdir / "sample_boundary_event_overlap_v9.tsv.gz"
    ...
    burden_path = outdir / "sample_mechanistic_burden_v9.tsv"
    ...
    summary_path = outdir / "sample_mechanistic_burden_summary_v9.tsv"
