#!/usr/bin/env python3
"""
14_compute_tad_domains_v3.py

v1 → v3 変更点 (ChatGPT Block 1+2 修正):
- Block 1: anchor 定義を `raw_value_max >= 2` から `group_primary in ['Diff_any','Static']`
  に変更 (CONTEXT の "≥2 atlas observation units" 要件は raw_value_max では不十分なので、
  bin_l2_annotation の事前計算済み Diff_any/Static label を直接使用)
- Block 2: 旧 v1 で未使用だった HEFFEL_MASTER / L2_CLASSES を削除して動作を明示化

処理内容:
- bin_l2_annotation_v2 の group_primary == 'Diff_any' or 'Static' の bin を boundary anchor
  として使用 (これら 2 set の union が "any boundary" 集合)
- 隣接 anchor 間の genomic interval を TAD domain として定義
- 各 boundary bin に left/right flanking domain を mapping
...
- output_v1/bin_to_flanking_domains_v3.tsv
...
# ============================================================
# Step 2: Define boundary anchors (FIXED in v3)
# ============================================================
# v3 Block 1 fix: use group_primary directly (bin_l2_annotation already encodes
# the "≥2 atlas observation units" requirement via the Diff_any / Static labels).
bin_df['is_boundary_anchor'] = bin_df['group_primary'].isin(['Diff_any', 'Static']).astype(int)
n_anchor = bin_df['is_boundary_anchor'].sum()
print(f"  Boundary anchors (group_primary in [Diff_any, Static]): {n_anchor}")
print(f"    Diff_any: {(bin_df['group_primary'] == 'Diff_any').sum()}")
print(f"    Static: {(bin_df['group_primary'] == 'Static').sum()}")
...
for chrom, sub in bin_df.groupby('chrom', sort=False):
    ...
    for i in range(len(anchor_idx) - 1):
        ...
        domains.append({
            ...
            'left_group_primary': left_anchor['group_primary'],
            'right_group_primary': right_anchor['group_primary'],
...
out_bed = OUT_DIR / 'tad_domains_v3.bed'
...
out_bin_dom = OUT_DIR / 'bin_to_flanking_domains_v3.tsv'
