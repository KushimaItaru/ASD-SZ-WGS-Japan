#!/usr/bin/env python3
"""
14_compute_per_sample_burden_v9.py

処理内容 (v7 → v9 変更点):
- direct-overlap exclusion を **proxy → exact** に変更:
  - WGS event_bin: sv_chr (col 4), sv_start0 (col 5), sv_end (col 6) を直接使用
  - arrayCGH event_bin: event_id (col 3, e.g., 'A134:chr2:106439107-107874624:DEL')
    を parse して sv_chr / sv_start / sv_end を抽出
  - MSSNG event_bin: v19 patch v4 で追加された sv_chr / sv_start / sv_end 列を使用
- 判定ロジック: gene_start < sv_end AND gene_end > sv_start (interval overlap)
  → flanking domain 内 constrained gene のうち、deletion が直接 overlap する
    gene を集計から除外。
- v8 fit は `_excl` variant をそのまま読むので、列名は v7 と同じ
  (`N_constr_strict_diffany_bounded_excl` 等) 。論文では
  「exact direct-overlap exclusion sensitivity」として記載できる。
- 5 burden variant 構成は v7 から継続:
  (1) diffany_bounded:        Diff_any-bounded domain (PRIMARY)
  (2) prox250kb / prox500kb / prox1000kb (boundary-proximal sensitivity)
  (3) diffany_bounded_excl:   PRIMARY - exact direct-overlap exclusion (v9 NEW)

入力:
- output_v1/bin_to_diffany_bounded_domains_v7.tsv (v7)
- output_v1/genes_per_domain_v4.tsv.gz (v4)
...
- arrayCGH event-bin: 08_arraycgh_sample_burden/output_v22/sample_event_bin_overlap_v22.tsv.gz
  (列: sample_id, bin_id, event_id, sv_type, pattern; event_id format:
...
- MSSNG event-bin: output_v1/mssng_event_bins_dumped_v1.tsv.gz
  (v19 patch v4: sample_id, bin_id, sv_type, pattern, Diagnosis, sv_chr, sv_start, sv_end)
- MSSNG sample cov: output_v1/mssng_sample_covariates_dumped_v1.tsv.gz

出力 (3 cohort files; 各 file に 5 burden variant の N_* 列, v9):
- output_v1/sample_constraint_burden_discovery_v9.tsv
- output_v1/sample_constraint_burden_arraycgh_v9.tsv
- output_v1/sample_constraint_burden_mssng_v9.tsv
...
# v9: gene_id -> (chrom, gene_start, gene_end) lookup (for exact direct-overlap exclusion)
GENE_ID_TO_INTERVAL = {}
...
def aggregate_burden_per_sample(df, sample_col, bin_col, sv_chr_col, sv_start_col, sv_end_col,
    ...
    v9 では `diffany_bounded_excl` variant を **exact direct-overlap exclusion**
    (gene_start < sv_end AND gene_end > sv_start) に変更。

    Args:
        df: event-bin records (sample × bin × ... × SV interval)
        sample_col: sample id column name
        bin_col: bin id column name
        sv_chr_col / sv_start_col / sv_end_col: SV interval column names
            None を渡すと proxy fallback (bin-level) になるが、v9 では None を許可しない.

    Variants:
      (1) diffany_bounded:        Diff_any-bounded domain (PRIMARY)
      (2) prox250kb / prox500kb / prox1000kb: ±X kb gene neighborhood
      (3) diffany_bounded_excl:   PRIMARY - exact direct-overlap exclusion
    """
    ...
    if any(c is None for c in [sv_chr_col, sv_start_col, sv_end_col]):
        raise RuntimeError(
            f"v9 requires SV interval columns (sv_chr/sv_start/sv_end). "
            f"None passed for {label}."
        )

    rows = []
    for sample, sub in df.groupby(sample_col):
        # Set of disrupted bins + per-bin SV intervals (Diff_any only)
        # Note: 同じ (sample, bin) でも複数 SV を含み得る (v9 dump は per-record)
        disrupted_bin_ids = []
        ...
        # ----- Variant 5: diffany_bounded_excl (v9: exact direct-overlap exclusion) -----
        # Exact direct-overlap: gene_start < sv_end AND gene_end > sv_start
        # (interval overlap between gene body and any disrupted SV)
...
out_wgs = OUT_DIR / 'sample_constraint_burden_discovery_v9.tsv'
...
# v9: parse event_id to extract SV interval (sv_chr, sv_start, sv_end)
# event_id format: 'A134:chr2:106439107-107874624:DEL'
...
out_acgh = OUT_DIR / 'sample_constraint_burden_arraycgh_v9.tsv'
...
# v9: require SV interval columns from v19 patch v4
...
out_mssng = OUT_DIR / 'sample_constraint_burden_mssng_v9.tsv'
