#!/usr/bin/env python3
"""
14_compute_per_sample_burden_v7.py

処理内容 (v6 → v7 変更点):
- TAD domain を Diff_any-bounded TAD-scale regulatory domain に変更 (v7 input)
- ChatGPT 推奨に従い、5 種類の burden variant を **同一スクリプトで生成**:
  (1) diffany_bounded:        Diff_any-bounded domain (PRIMARY)
  (2) prox250kb:              Diff_any boundary midpoint ±250 kb 内 gene (sensitivity)
  (3) prox500kb:              Diff_any boundary midpoint ±500 kb 内 gene (sensitivity, primary supplement)
  (4) prox1Mbkb:              Diff_any boundary midpoint ±1 Mb 内 gene (sensitivity)
  (5) diffany_bounded_excl:   Primary - direct-overlap exclusion (proxy:
                              gene midpoint が disrupted bin の中にある場合除外)
- 各 variant について、3 cohort (WGS, arrayCGH, MSSNG) 別に出力
- v6 から継承:
  - WGS/arrayCGH/MSSNG 全 sample に covariate を left merge
  - MSSNG Platform/ancestry を pd.get_dummies で sanitize 化
  - LOEUF unknown を unconstrained に混入させない
  - safe_strip_ab_suffix(), required cov NaN hard fail, anti-join check

入力:
- output_v1/bin_to_diffany_bounded_domains_v7.tsv (v7: Diff_any anchor only)
- output_v1/genes_per_domain_v4.tsv.gz (v4: input bed v7)
- (gene 全座標が必要なので) /lustre12/home/kushima-pg/annotationInfo/gencode.v46.annotation.gtf.gz
...
- output_v1/sample_constraint_burden_discovery_v7.tsv
- output_v1/sample_constraint_burden_arraycgh_v7.tsv
- output_v1/sample_constraint_burden_mssng_v7.tsv

各 file は per-sample × multi-variant N_* columns:
  N_constr_strict_diffany_bounded, N_unconstr_strict_known_diffany_bounded, ...
  N_constr_strict_prox250kb, ...
  N_constr_strict_prox500kb, ...
  N_constr_strict_prox1000kb, ...
  N_constr_strict_diffany_bounded_excl, ...

CAVEAT (direct-overlap exclusion proxy の意味):
  diffany_bounded_excl variant は **boundary-bin midpoint exclusion proxy** で、
  gene midpoint が disrupted 25-kb boundary bin の中に入る場合のみ除外する。
  これは「deletion が gene 全体を直接 overlap する」厳密な direct-overlap exclusion
  ではない。event-bin file が SV interval (sv_start, sv_end) を持たないため、
  exact direct-overlap exclusion は実装上不可能。
  論文では「direct deletion overlap exclusion」と呼ばず、
  「boundary-bin-localized gene-midpoint exclusion sensitivity」として記載すること。
...
BIN_DOM = OUT_DIR / 'bin_to_diffany_bounded_domains_v7.tsv'
GENES_PER_DOM = OUT_DIR / 'genes_per_domain_v4.tsv.gz'
...
# ============================================================
# Step 1: Load TAD domain mapping (v7)
# ============================================================
print(f"[{time.time()-t0:.1f}s] Loading bin-to-Diff_any-bounded-domain mapping (v7) ...")
...
# ============================================================
# Step 2: Load gene table for ±X kb neighborhood + direct-overlap proxy
# ============================================================
...
# Constraint flags (consistent with annotate_v4)
...
def gene_count_record(gene_records):
    """Aggregate gene flags into per-sample record fields (with given tag)."""
...
def aggregate_burden_per_sample(df, sample_col, bin_col, label):
    """For each sample, compute 5 burden variants over Diff_any disrupted bins.
    Returns DataFrame with columns 'sample_id' + N_*_<variant> for each variant.
    Variants:
      - diffany_bounded: gene union over flanking Diff_any-bounded domains (PRIMARY)
      - prox250kb / prox500kb / prox1Mb: ±X kb gene neighborhood (sensitivity)
      - diffany_bounded_excl: primary - direct-overlap exclusion (proxy via bin co-localization)
    """
    print(f"[{time.time()-t0:.1f}s] Aggregating 5-variant burden for {label} "
          f"({len(df)} records) ...")
    rows = []
    for sample, sub in df.groupby(sample_col):
        # Set of disrupted bins (filtered to Diff_any only)
        disrupted_bin_ids = []
        for bid in sub[bin_col].unique():
            info = BIN_INFO.get(bid)
            if info is None:
                continue
            gp, chrom, b_start, b_end, ldid, rdid = info
            if gp != 'Diff_any':
                continue
            disrupted_bin_ids.append((bid, chrom, b_start, b_end, ldid, rdid))

        # ----- Variant 1: diffany_bounded (PRIMARY) -----
        seen_genes_dom = {}  # gene_id -> record
        seen_dom_ids = set()
        for bid, chrom, b_start, b_end, ldid, rdid in disrupted_bin_ids:
        ...
        # ----- Variant 5: diffany_bounded_excl_direct (PRIMARY - direct-overlap proxy) -----
        # Direct-overlap proxy: gene の midpoint が disrupted bin の中に入っている場合除外
        # (gene が deletion 由来 bin に直接重なるかを bin-level で proxy 判定)
        # 厳密な direct overlap (gene_start, gene_end vs deletion_start, deletion_end) は
        # event_bin file が SV interval を持たないので不可。Bin-level proxy として実装。
        bin_intervals_by_chrom = defaultdict(list)
        for bid, chrom, b_start, b_end, ldid, rdid in disrupted_bin_ids:
            bin_intervals_by_chrom[chrom].append((b_start, b_end))
        # union sort
        for chrom in bin_intervals_by_chrom:
            iv = sorted(bin_intervals_by_chrom[chrom])
            bin_intervals_by_chrom[chrom] = iv

        def gene_mid_in_disrupted_bin(gene_id):
            # find gene's chrom, midpoint, return True if mid in any disrupted bin
            return False  # placeholder, replaced below by gene context

        # Build a set of gene_ids whose midpoint falls inside any disrupted bin
        # For domain primary genes only
        excluded_gene_ids = set()
        # Need gene midpoints; gene records carry 'gene_id' and 'gene_symbol' but no
        # midpoint. We retrieve from gene_df via a lookup.
        # (Pre-built lookup: gene_id_to_chrom_mid)
        for gid in seen_genes_dom:
            chrom_mid = GENE_ID_TO_CHROM_MID.get(gid)
            if chrom_mid is None:
                continue
            chrom, gmid = chrom_mid
            for b_start, b_end in bin_intervals_by_chrom.get(chrom, []):
                if b_start <= gmid < b_end:
        ...
        for bid, chrom, b_start, b_end, ldid, rdid in disrupted_bin_ids:
            chrom_data = GENE_BY_CHR.get(chrom)
        ...
        for k, v in gene_count_record(list(seen_genes_dom.values())).items():
            record[f'{k}_diffany_bounded'] = v
        # Variant 5: diffany_bounded_excl_direct
        for k, v in gene_count_record(list(seen_genes_dom_excl.values())).items():
            record[f'{k}_diffany_bounded_excl'] = v
        record['n_excluded_direct_overlap'] = len(excluded_gene_ids)
        # Variants 2/3/4: prox<kb>
        for kb in NEIGHBORHOOD_KB:
...
# Pre-build gene_id -> (chrom, midpoint) lookup
GENE_ID_TO_CHROM_MID = {}
for chrom, data in GENE_BY_CHR.items():
    for i, mid in enumerate(data['gene_mid']):
        GENE_ID_TO_CHROM_MID[data['records'][i]['gene_id']] = (chrom, int(mid))
...
exposure_wgs_raw = aggregate_burden_per_sample(wgs_filt, 'sample_id', 'bin_id', 'discovery_wgs')
...
out_wgs = OUT_DIR / 'sample_constraint_burden_discovery_v7.tsv'
exposure_wgs.to_csv(out_wgs, sep='\t', index=False)
print(f"  Saved {out_wgs}")
# Sanity per variant
for var in ['diffany_bounded', 'prox250kb', 'prox500kb', 'prox1000kb', 'diffany_bounded_excl']:
...
exposure_acgh_raw = aggregate_burden_per_sample(acgh_filt, 'sample_id', 'bin_id', 'arraycgh')
...
out_acgh = OUT_DIR / 'sample_constraint_burden_arraycgh_v7.tsv'
...
exposure_mssng_raw = aggregate_burden_per_sample(mssng_filt, 'sample_id', 'bin_id', 'mssng')
...
out_mssng = OUT_DIR / 'sample_constraint_burden_mssng_v7.tsv'
