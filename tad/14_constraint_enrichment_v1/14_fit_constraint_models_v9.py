#!/usr/bin/env python3
"""
14_fit_constraint_models_v9.py

処理内容 (v8 → v9 変更点):
- 入力 burden を v9 (sample_constraint_burden_*_v9.tsv) に切替
- output を constraint_enrichment_results_v9.tsv に切替
- v9 では `diffany_bounded_excl` variant が **exact direct-overlap exclusion**
  (gene_start < sv_end AND gene_end > sv_start) になっており、論文では
  「exact direct-overlap exclusion sensitivity」として記載できる
- 統計設計・require_fit_ok / Block 1, 2 fix 等は v8 から不変

v8 で導入された fix (継承; ChatGPT v7 review Block 1, 2 fix):
...
入力:
- output_v1/sample_constraint_burden_discovery_v9.tsv (burden v7 出力)
- output_v1/sample_constraint_burden_arraycgh_v9.tsv
- output_v1/sample_constraint_burden_mssng_v9.tsv

出力:
- output_v1/constraint_enrichment_results_v9.tsv
...
EXPOSURE_DISC = OUT_DIR / 'sample_constraint_burden_discovery_v9.tsv'
EXPOSURE_ACGH = OUT_DIR / 'sample_constraint_burden_arraycgh_v9.tsv'
EXPOSURE_MSSNG = OUT_DIR / 'sample_constraint_burden_mssng_v9.tsv'
...
# Save results
# ============================================================
out_results = OUT_DIR / 'constraint_enrichment_results_v9.tsv'
