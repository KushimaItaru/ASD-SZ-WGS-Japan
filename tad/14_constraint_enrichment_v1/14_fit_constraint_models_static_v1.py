#!/usr/bin/env python3
"""
14_fit_constraint_models_static_v1.py

処理内容 (v231 round, M3 対応 — Static-bounded null control):
- v9 (Diff_any-bounded) と完全に同一の B' framework / GEE / IVW / contrast / Holm
  ロジックを Static-bounded burden に適用 (M3 反論用 null control)。
- 違いは入出力ファイル名と variant ラベルのみ。
   * 入力: sample_constraint_burden_*_static_v1.tsv
   * 出力: constraint_enrichment_results_static_v1.tsv
   * Variant ラベル: StaticBounded_LOEUF0.35 (etc.)
   * Holm 補正は同じ pre-specified primary 2-test family
     (Test 1 WGS + Test 4 External IVW Meta) で適用。
- 期待結果: Static-bounded で OR ≈ 1 / P > 0.1 が得られれば
  reviewer M3 反論「Diff_any anchor → Diff_any-bounded domain → enrichment は circular」
  に対して、「Static anchor では同じ B' framework / cohort 構成下でも signal が
  attenuate する」という直接的な反論が可能。
...
出力:
- output_v1/constraint_enrichment_results_static_v1.tsv
...
EXPOSURE_ACGH = OUT_DIR / 'sample_constraint_burden_arraycgh_static_v1.tsv'
EXPOSURE_MSSNG = OUT_DIR / 'sample_constraint_burden_mssng_static_v1.tsv'
...
# Load exposures
# ============================================================
print(f"[{time.time()-t0:.1f}s] Loading static_v1 exposure files (M3 null control) ...")
df_disc = pd.read_csv(EXPOSURE_DISC, sep='\t')
df_acgh = pd.read_csv(EXPOSURE_ACGH, sep='\t')
df_mssng = pd.read_csv(EXPOSURE_MSSNG, sep='\t')

assert_cols(df_disc, ['sample_id', 'Diagnosis', 'N_constr_strict_static_bounded'] + WGS_COVS, 'WGS')
assert_cols(df_acgh, ['sample_id', 'Diagnosis', 'N_constr_strict_static_bounded',
                      'platform_nimblegen'] + ACGH_COVS_STRAT, 'arrayCGH')
assert_cols(df_mssng, ['sample_id', 'Diagnosis', 'N_constr_strict_static_bounded']
...
# ============================================================
# Burden variant configurations (Static-bounded null control)
# ============================================================
# (variant_suffix, exposure_constr_col, exposure_unconstr_col, label_prefix, primary_flag)
VARIANTS = [
    ('static_bounded', 'N_constr_strict_static_bounded', 'N_unconstr_strict_known_static_bounded',
     'StaticBounded_LOEUF0.35', True),  # PRIMARY null control
    ('prox500kb', 'N_constr_strict_prox500kb', 'N_unconstr_strict_known_prox500kb',
     'StaticProx500kb_LOEUF0.35', False),
    ('prox250kb', 'N_constr_strict_prox250kb', 'N_unconstr_strict_known_prox250kb',
     'StaticProx250kb_LOEUF0.35', False),
    ('prox1000kb', 'N_constr_strict_prox1000kb', 'N_unconstr_strict_known_prox1000kb',
     'StaticProx1Mb_LOEUF0.35', False),
    ('static_bounded_excl', 'N_constr_strict_static_bounded_excl',
     'N_unconstr_strict_known_static_bounded_excl',
     'StaticBounded_DirectExcl_LOEUF0.35', False),
]

# Sensitivity cutoffs (only on primary static_bounded variant)
CUTOFF_SENSITIVITIES = [
    ('relaxed', 'N_constr_relaxed_static_bounded', 'N_unconstr_relaxed_known_static_bounded',
     'StaticBounded_LOEUF0.6'),
    ('pli', 'N_pLI_constr_static_bounded', 'N_pLI_unconstr_known_static_bounded',
     'StaticBounded_pLI0.9'),
]
...
# (Test 1 WGS StaticBounded LOEUF<0.35) + (Test 4 IVW Meta StaticBounded LOEUF<0.35)
# ============================================================
...
if primary_r1 and primary_meta:
    ...
    if not (np.isnan(p1) or np.isnan(p4)):
        ...
        results.append({'label': 'Holm_PrimaryFamily_Test1_WGS_StaticBounded_LOEUF0.35',
                         'p_one_sided': p1, 'p_holm_adjusted': float(p_holm[0])})
        results.append({'label': 'Holm_PrimaryFamily_Test4_Meta_StaticBounded_LOEUF0.35',
...
# ============================================================
print(f"\n[{time.time()-t0:.1f}s] === Contrast IVW external meta (PRIMARY StaticBounded) ===")
...
primary_label_prefix = 'StaticBounded_LOEUF0.35'
...
if c_a and c_n:
    ...
    if acgh_contrast_iv.get('fit_status') == 'ok':
        rec = {'label': 'arrayCGH_within-array_contrast_IVW_StaticBounded_supporting'}
        ...
        if c_m:
            ...
            if ext_contrast.get('fit_status') == 'ok':
                rec2 = {'label': 'Test4_contrast_external_IVW_meta_StaticBounded_supporting'}
...
# Save results
# ============================================================
out_results = OUT_DIR / 'constraint_enrichment_results_static_v1.tsv'
