#!/usr/bin/env python3
"""
14_compute_per_sample_burden_v5.py

処理内容 (v4 → v5 変更点; ChatGPT v4 review safety patch):
- Priority 1-2: MSSNG required covariate (Sex, log1p_total_del_bases,
  log1p_total_gene_DEL) の NaN を **hard fail** に変更 (v4 では warning のみ)
  FAMILYID は _solo_<sid> fallback OK
- Priority 1-3 (preparation): dummy column 名を sanitize (英数字+`_` のみ)。
  fit script 側で formula API → matrix API に変える前提で、それでも安全になるよう
  dummy 名を robust 化。
- Anti-join check: event-bin sample が cov dump にない場合の hard fail を追加
  (両ファイルが同じ v19 patch から出るので 0 のはずだが、0 でなければ denominator が
  壊れているので明示的に止める)
- 既存修正の継承 (v4 から):
  - WGS/arrayCGH/MSSNG 全 sample に covariate を left merge (Block 1)
  - MSSNG Platform/ancestry を pd.get_dummies で dummy 化 (Block 2)
  - LOEUF unknown を unconstrained に混入させない
  - safe_strip_ab_suffix() で 'AB' suffix のみ安全削除
  - dedup by gene_id, not symbol

入力:
- output_v1/bin_to_flanking_domains_v3.tsv (v3, anchor logic 不変)
- output_v1/genes_per_domain_v2.tsv.gz (v2, LOEUF 分離不変)
...
出力:
- output_v1/sample_constrained_burden_discovery_v5.tsv
- output_v1/sample_constrained_burden_arraycgh_v5.tsv
- output_v1/sample_constrained_burden_mssng_v5.tsv (Platform/ancestry sanitized dummies 含む)
...
BIN_DOM = OUT_DIR / 'bin_to_flanking_domains_v3.tsv'
GENES_PER_DOM = OUT_DIR / 'genes_per_domain_v2.tsv.gz'
...
out_wgs = OUT_DIR / 'sample_constrained_burden_discovery_v5.tsv'
...
out_acgh = OUT_DIR / 'sample_constrained_burden_arraycgh_v5.tsv'
...
out_mssng = OUT_DIR / 'sample_constrained_burden_mssng_v5.tsv'
