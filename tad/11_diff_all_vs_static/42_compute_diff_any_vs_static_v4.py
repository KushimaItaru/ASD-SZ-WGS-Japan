#!/usr/bin/env python3
# 42_compute_diff_any_vs_static_v4.py
#
# 処理内容:
...
#      *命名は 3 exposure で "Diff_any_DEL" に統一 (ChatGPT レビュー反映、v2 の Diff_all を廃止)
#  - Fit & meta:
...
#                *v4: QR-with-pivoting で rank-deficient 列を自動検出・drop
#      Meta    : IVW fixed-effect + DerSimonian-Laird random-effects
#  - 6 tests (3 exposure × 2 category=Diff_any,Static) に BH + Holm 補正
#  - 出力: meta_diff_any_vs_static_v4.tsv (6 行、列は fixed+RE+Firth+FDR)
#  - 処理時間を先頭と末尾で記録
#
# v3 -> v4 変更点:
#  (v3 実行で Firth が全 6 行 singular_hessian fail した問題への対応、2026-04-21)
#  1. Firth に QR-with-pivoting ベースの rank reduction を追加
#     - MSSNG design matrix では platform one-hot (plat_* 5 列) と intercept
#       (sum=1) が完全共線性のため X'WX が rank deficient、np.linalg.inv が失敗
#       していたのを修正
#     - intercept (col 0) と exposure (col 1) を protected columns として必ず保持
#     - pivot 先頭 rank 個に protected を union、rank が合わない場合は
#       非 protected 末尾から drop
#     - 反復計算は reduced X 上で実施、full-size beta/se 配列に戻して返却
#  2. 新規ヘルパー関数: _reduce_rank_via_qr(X, protected_idx, tol)
#  3. firth_logit_fit に protected_idx=(0, 1) を追加
#  4. OUT_DIR: output_v3 -> output_v4
#  5. 出力ファイル名: meta_diff_any_vs_static_v3.tsv -> meta_diff_any_vs_static_v4.tsv
#  6. Script metadata / log banner を v4 に統一
#  7. Main 解析ロジック (arrayCGH Logit, MSSNG GEE, IVW fixed/DL random-effects,
#     BH/Holm 補正, Diff_any 列加算) は v3 と完全に同一
...
#SBATCH --output=42_compute_diff_any_vs_static_v4_%j.out
#SBATCH --error=42_compute_diff_any_vs_static_v4_%j.err
...
OUT_DIR = Path(
    "/lustre12/home/kushima-pg/tad04212026/11_diff_all_vs_static/output_v4"
)
OUT_TSV = OUT_DIR / "meta_diff_any_vs_static_v4.tsv"


# =========================================================
# 定数 (v3/v4 pipeline と一致)
# =========================================================
...
def add_diff_any_columns(df: pd.DataFrame, prefix: str) -> pd.DataFrame:
    """
    df に Diff_any_DEL 列を exposure-aware に追加:
      - n_boundary, n_events: specific + shared (count sum、従来 Diff_all と同じ)
      - carrier_boundary    : (specific>0) OR (shared>0) の binary (0/1) clip
                              → "Dynamic_any carrier" の真の定義
    """
    out = df.copy()
...
# =========================================================
# Firth penalized logistic regression (自前実装)
# =========================================================
def _reduce_rank_via_qr(
    ...
    """
    QR with pivoting を使って X の rank deficient 列を検出し、drop 後の
    design matrix と残した列インデックスを返す。
    protected_idx は必ず保持する列 (intercept=0, exposure=1 等)。

    Returns
    -------
    X_red    : (n, k) ndarray, rank-reduced design matrix
    keep_idx : (k,) ndarray of int (column indices of original X that are kept)
    reduced  : bool, True if any column was dropped
    """
    n, p = X.shape
    # scale columns so that tolerance is meaningful
    col_norms = np.linalg.norm(X, axis=0)
    col_norms_safe = np.where(col_norms > 0, col_norms, 1.0)
    X_scaled = X / col_norms_safe

    try:
        # scipy.linalg.qr with pivoting=True returns (Q, R, pivot)
    ...
    if diag_R.size == 0:
        # degenerate: keep only protected
    ...
    # start with the first `rank` pivoted columns
    keep_set = set(pivot[:rank].tolist())
    # force-include protected columns
    for pi in protected_idx:
        if 0 <= pi < p:
            keep_set.add(pi)
    keep_sorted = sorted(keep_set)

    # if we over-added protected, drop non-protected until rank matches
    if len(keep_sorted) > rank:
        ...
        # iterate from end (last added) and drop until full rank
...
def firth_logit_fit(
    ...
    """
    Firth's penalized likelihood logistic regression (Firth 1993).
      U_penalized = X' (y - mu + (h/2) * (1 - 2*mu))
      h = diag( W^(1/2) X (X'WX)^(-1) X' W^(1/2) )  [hat matrix diagonal]
      W = diag(mu * (1 - mu))
    Newton-Raphson with modified score. Standard errors from unpenalized
    Fisher information at convergence (as in Heinze & Schemper 2002).

    v4: QR with pivoting で rank-deficient 列を事前 drop (protected_idx は保持)。

    Parameters
    ----------
    X : (n, p) ndarray, design matrix INCLUDING intercept (col 0) and exposure (col 1)
    y : (n,) ndarray, binary outcome
    maxiter, tol: Newton-Raphson stopping criteria
    protected_idx: columns that must NOT be dropped (default: intercept + exposure)

    Returns
    -------
    beta : (p,) ndarray of coefficients in ORIGINAL index (NaN for dropped cols)
    se   : (p,) ndarray of standard errors (NaN for dropped cols)
    p    : (p,) ndarray of two-sided p-values (NaN for dropped cols)
    converged : bool
    status : str ("ok", "ok_rank_reduced:<dropped_count>",
                  "not_converged", "singular_hessian", "error:<msg>")
    """
    n, p_full = X.shape
    try:
        # Step 1: rank reduction via QR
        X_red, keep_idx, reduced = _reduce_rank_via_qr(
            X, protected_idx=protected_idx
        )
        p = X_red.shape[1]

        # make sure protected columns (especially exposure=1) survived
        for pi in protected_idx:
        ...
        # Step 2: Newton-Raphson Firth on reduced X
        ...
        if not converged:
            # return best-effort estimate with status
            pass

        # Step 3: SE from unpenalized Fisher info at convergence
        eta = np.clip(X_red @ beta, -30.0, 30.0)
        mu = 1.0 / (1.0 + np.exp(-eta))
        mu = np.clip(mu, 1e-10, 1.0 - 1e-10)
        W = mu * (1.0 - mu)
        ...
        # Step 4: map reduced-size back to full-size (NaN for dropped cols)
...
def fit_firth_mssng_pooled(
    ...
    """
    MSSNG Firth pooled logistic (family clustering を無視した sparse-control sensitivity)。
    covariates は GEE と同一 (MSSNG_COVS_BASE + anc_* + plat_*)。
    """
    sub = df[df["Status"].astype(str).isin([case_label, control_label])].copy()
    sub["is_case"] = (sub["Status"].astype(str) == case_label).astype(int)
    anc_cols = [c for c in MSSNG_ANCESTRY if c in sub.columns]
...
def dl_random_effects_meta(beta_a, se_a, beta_b, se_b) -> dict:
    """
    DerSimonian-Laird (1986) random-effects meta-analysis for 2 studies.
      Q = sum_i w_i * (beta_i - beta_FE)^2
      C = sum(w_i) - sum(w_i^2)/sum(w_i)
      tau^2 = max(0, (Q - (k-1)) / C)  [k=2 here]
      w_i_RE = 1 / (se_i^2 + tau^2)
      beta_RE = sum(w_i_RE * beta_i) / sum(w_i_RE)
      se_RE   = sqrt(1 / sum(w_i_RE))
    """
    if any(pd.isna([beta_a, se_a, beta_b, se_b])):
        return {
            "beta_meta_re": np.nan,
...
def main() -> None:
    t0 = time.time()
    log("=" * 70)
    log("42_compute_diff_any_vs_static_v4.py START")
    log("=" * 70)
    ...
    for exp in EXPOSURES:
        for cat in CATEGORIES:
            ...
            # MSSNG Firth pooled logistic (sensitivity)
            ...
            # DL random-effects meta (sensitivity)
    ...
    # --- Multiple testing correction (6 tests: 3 exposure × 2 category)
    #   main: p_meta (fixed-effect)
    #   sensitivity: p_meta_re (random-effect)
    ...
    # p_meta の one-sided も付与 (Diff_any は discovery-positive 側、Static は内部参照)
    ...
    t1 = time.time()
    log("=" * 70)
    log(f"42_compute_diff_any_vs_static_v4.py DONE in {t1 - t0:.1f} sec")
    log("=" * 70)
