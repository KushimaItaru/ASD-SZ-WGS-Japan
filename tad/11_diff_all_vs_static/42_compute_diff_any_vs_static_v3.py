#!/usr/bin/env python3
# 42_compute_diff_any_vs_static_v3.py
#
# 処理内容:
...
#  - Fit & meta:
#      arrayCGH: B' pooled logistic (statsmodels.Logit) [GEE の代替なし、main]
...
#  - 出力: meta_diff_any_vs_static_v3.tsv (6 行、列は fixed+RE+Firth+FDR)
#  - 処理時間を先頭と末尾で記録
#
# v2 -> v3 変更点:
#  (ChatGPT レビュー反映、2026-04-21)
#  1. 命名統一: Diff_all_DEL -> Diff_any_DEL (3 exposure 全て)
#  2. carrier_boundary の集約: sum (0/1/2) -> binary OR (0/1)
#     *論文 "Dynamic_any carrier" semantics と整合させるため
#  3. 0/1/2 版 (v2 Diff_all) は廃止 (ユーザー判断 by Q3)
#  4. Random-effects meta (DerSimonian-Laird tau^2) を追加
#  5. BH + Holm multiple testing 補正を追加 (6 tests 一律)
#  6. MSSNG Firth penalized logistic sensitivity を追加 (pooled, 自前実装)
#  7. OUT_DIR: output_v2 -> output_v3
#  8. 出力ファイル名: meta_diff_all_vs_static_v2.tsv -> meta_diff_any_vs_static_v3.tsv
...
#SBATCH --output=42_compute_diff_any_vs_static_v3_%j.out
#SBATCH --error=42_compute_diff_any_vs_static_v3_%j.err
...
OUT_DIR = Path(
    "/lustre12/home/kushima-pg/tad04212026/11_diff_all_vs_static/output_v3"
)
OUT_TSV = OUT_DIR / "meta_diff_any_vs_static_v3.tsv"
...
def firth_logit_fit(
    ...
    Parameters
    ----------
    X : (n, p) ndarray, design matrix INCLUDING intercept
    y : (n,) ndarray, binary outcome
    maxiter, tol: Newton-Raphson stopping criteria

    Returns
    -------
    beta : (p,) ndarray of coefficients (NaN if failed)
    se   : (p,) ndarray of standard errors (NaN if failed)
    p    : (p,) ndarray of two-sided p-values (NaN if failed)
    converged : bool
    status : str ("ok", "not_converged", "singular_hessian", "error:<msg>")
    """
    n, p = X.shape
    beta = np.zeros(p)
    converged = False
    ...
    try:
        for it in range(maxiter):
            ...
            # hat matrix diagonal: h_i = (W_i)^{1/2} * x_i' * (X'WX)^{-1} * x_i * (W_i)^{1/2}
            Wsqrt = np.sqrt(W)
            XWhalf = X * Wsqrt[:, None]
            H_diag = np.einsum("ij,jk,ik->i", XWhalf, XWX_inv, XWhalf)
            # modified score
            U = X.T @ (y - mu + (H_diag / 2.0) * (1.0 - 2.0 * mu))
            delta = XWX_inv @ U
            beta_new = beta + delta
            if np.max(np.abs(delta)) < tol:
                ...
                break
            beta = beta_new
        # final Fisher info (unpenalized) for SE
        eta = np.clip(X @ beta, -30.0, 30.0)
        mu = 1.0 / (1.0 + np.exp(-eta))
        mu = np.clip(mu, 1e-10, 1.0 - 1e-10)
        W = mu * (1.0 - mu)
        ...
        return beta, se, pvals, converged, status
    except Exception as e:
        return (
            np.full(p, np.nan),
            np.full(p, np.nan),
...
def main() -> None:
    t0 = time.time()
    log("=" * 70)
    log("42_compute_diff_any_vs_static_v3.py START")
    log("=" * 70)
    ...
    t1 = time.time()
    log("=" * 70)
    log(f"42_compute_diff_any_vs_static_v3.py DONE in {t1 - t0:.1f} sec")
    log("=" * 70)
