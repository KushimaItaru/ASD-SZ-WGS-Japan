#!/usr/bin/env python3
# compute_loco_diff_any_sensitivity_v2.py
#
# Purpose:
#   Leave-One-Class-Out (LOCO) Diff_any union sensitivity for R18 aggregate primary.
#   Test whether aggregate Diff_any signal is driven by class-unique contributions
#   from any one L2 boundary class.
#
...
#   Expectation: 10 LOCO ORs within tight range of baseline supports
#                "not driven by class-unique contributions".
#
# v1 -> v2 changes (important):
#   v1 used "subset-level 99-percentile high-burden CNV outlier filter".
#   v2 removes this. Reason: Step 03 v9 (34a_extract_rare_sv_events_v9.py) already
#   excludes top-1% cnv_count samples via Global flag from all event tables.
#   Step 04 v10 / Step 05 v3 burden TSV is post-Global-flag. Adding subset-level
#   99-percentile filter creates double-filtering and post hoc subset-specific
#   inclusion criteria. Removed in v2. Step 04 v10 overlap table is also
#   post-Global-flag. Result: ASD vs Healthy DEL N=8,877 (matches Step 07 v9).
#
# Inputs:
...
#                                                 Step 03 v9 + Step 04 v10 Global flag applied)
...
#     output/loco_diff_any_sensitivity_v2.tsv
#     output/loco_diff_any_sensitivity_v2.log
#
# Run:
#   cd /home/kushima-pg/tad04212026/16_aggregate_primary_sensitivity_v1/
#   python3 compute_loco_diff_any_sensitivity_v2.py
#
# Elapsed time logged.
import os
import sys
...
OUT_TSV = os.path.join(OUTDIR, "loco_diff_any_sensitivity_v2.tsv")
OUT_LOG = os.path.join(OUTDIR, "loco_diff_any_sensitivity_v2.log")
...
with open(OUT_LOG, "w") as logfh:
    log("=" * 70, logfh)
    log("compute_loco_diff_any_sensitivity_v2.py", logfh)
    ...
    log("  After covariates dropna only [Step 03 v9 Global flag inherited]: N=%d (ASD=%d, Healthy=%d)" % (
    ...
    if not loco_only.empty:
        ...
        log("  All LOCO ORs ranging within +/-%.1f%% of baseline: %s" % (
            ...
            "class-unique contributions do not drive the aggregate signal" if ors.min() > 1.0 else "check classes with OR<1"
        ), logfh)
