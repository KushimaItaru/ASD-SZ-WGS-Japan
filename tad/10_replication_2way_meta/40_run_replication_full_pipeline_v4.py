...
# -*- coding: utf-8 -*-
"""
40_run_replication_full_pipeline_v4.py
=====================================================
処理内容:
  - v4 (tad04212026 パイプライン移植): v3 からの最小差分修正版
      * すべての入力/出力パスを tad04212026 ツリーへ移行
          - ACGH 入力:
                noncoding_tad_arraycgh_03302026/output_tad_replication_arraycgh_v21/*_v21.*
             -> tad04212026/08_arraycgh_sample_burden/output_v22/*_v22.*
          - BIN_ANNOTATION:
                heffel_deep_analysis_03242026/bin_annotation_outputs_v1/bin_l2_annotation_v1.tsv.gz
             -> tad04212026/02_bin_l2_annotation/output_v2/bin_l2_annotation_v2.tsv.gz
          - WGS primary (Step D 用):
                heffel_deep_analysis_03242026/B_prime_primary_full_pipeline_outputs_v2/
                B_prime_(L2_classes|specificity_groups)_results_v2.tsv
             -> tad04212026/06_wgs_primary_L2/output_v3/
                B_prime_(L2_classes|specificity_groups)_results_v3.tsv
          - MSSNG 入力は read-only で据え置き:
                noncoding_tad_specificity_04022026/output_mssng_factorial_overlap_v2/
                mssng_sample_(event_bin_overlap|covariates)_v1.*
      * OUT_DIR:
          heffel_deep_analysis_03242026/replication_full_pipeline_outputs_v2
          -> tad04212026/10_replication_2way_meta/output_v4
      * 出力 TSV のファイル名サフィックスを _v1 -> _v4 に統一
      * Step A/B/C/D の処理ロジック、関数定義、列選択は v3 と同一

  - Replication (arrayCGH + MSSNG) の burden 計算 + B' logistic/GEE fit
    + IVW fixed-effect meta を 1 本にまとめた統合パイプライン。
    旧 40_compute_replication_burden_L2_and_specificity_v1.py (burden) と
    旧 41_fit_replication_B_prime_L2_and_specificity_v1.py (fit + meta) の後継。
  ...
  - Step A (burden):
      ...
      サンプル x (10 L2 classes + 5 group definitions) x DEL の
      bin-count / event-count / carrier burden を計算、共変量テーブルと merge。

  - Step B (fit):
      arrayCGH: B' pooled logistic (statsmodels.Logit)
                Logit(case) ~ exposure + sex + log1p_total_del_bases_A
                              + log1p_total_gene_DEL_A + platform_nimblegen
                comparison: ASD_vs_CONT / ASD_vs_SZ (SCZ)
      MSSNG   : GEE (Binomial + Independence, groups=FAMILYID)
                exposure + Sex_numeric + log1p_total_del_bases + log1p_total_gene_DEL
                + anc_* + plat_*
                comparison: ASD_vs_unaffSib (case=ASD, ctrl=unaffected_sibling)

  - Step C (2-way meta):
      arrayCGH ASD_vs_CONT beta/se と MSSNG ASD_vs_unaffSib beta/se を
      IVW fixed-effect (w = 1/SE^2) で統合。Cochran's Q / I^2 も計算。

  - Step D (3-way meta):
      WGS primary v3 (tad04212026 06_wgs_primary_L2 output_v3, ASD_vs_HC, DEL)
        + arrayCGH v22 (ASD_vs_CONT, DEL)
        + MSSNG (ASD_vs_unaffSib, DEL)
      を IVW fixed-effect で統合する 3-way メタ解析。N-cohort Cochran's Q / I^2 付き。
      WGS primary の結果は tad04212026/06_wgs_primary_L2/output_v3 の
      B_prime_L2_classes_results_v3.tsv / B_prime_specificity_groups_results_v3.tsv を
      読み込む。
  ...
  - 5 group definitions:
      group_primary / group_s2 / group_s3 / group_s4 / group_s5
  - 3 exposures: n_boundary / n_events / carrier_boundary
  - 実行時間記録あり

使い方:
  python 40_run_replication_full_pipeline_v4.py
  (引数不要。下の PATHS セクションを編集すればパスを差し替え可能。)

出力 (OUT_DIR = tad04212026/10_replication_2way_meta/output_v4 に配置):
  [burden]
    arraycgh_burden_L2_and_specificity_v4.tsv
    mssng_burden_L2_and_specificity_v4.tsv
    replication_burden_summary_v4.tsv
  [fit cohort-wise]
    arraycgh_L2_classes_results_v4.tsv
    arraycgh_specificity_groups_results_v4.tsv
    mssng_L2_classes_results_v4.tsv
    mssng_specificity_groups_results_v4.tsv
  [fit 2-way meta]
    meta_L2_classes_results_v4.tsv
    meta_specificity_groups_results_v4.tsv
  [fit 3-way meta (WGS v3 + arrayCGH v22 + MSSNG)]
    meta3way_L2_classes_results_v4.tsv
    meta3way_specificity_groups_results_v4.tsv
  [summary]
    replication_fit_summary_v4.tsv
"""
...
# =========================================================
# v4 CHANGE: ACGH 入力を tad04212026 v22 へ移行
ACGH_OVERLAP = Path(
...
# MSSNG 入力は read-only で据え置き (noncoding_tad_specificity_04022026)
MSSNG_OVERLAP = Path(
...
# v4 CHANGE: bin_annotation を tad04212026 Step 2 v2 出力へ切り替え
BIN_ANNOTATION = Path(
    "/lustre12/home/kushima-pg/tad04212026/02_bin_l2_annotation/"
    "output_v2/bin_l2_annotation_v2.tsv.gz"
)
# v4 CHANGE: WGS primary を tad04212026 Step 6 v3 出力へ切り替え
# 38_run_primary_full_pipeline_v2 (heffel) -> 39_fit_B_prime_L2_and_specificity_v3.R (tad04212026)
# どちらも comparison=ASD_vs_HC, sv_type=DEL のフィルタ仕様は同一。
WGS_PRIMARY_OUT_DIR = Path(
    "/lustre12/home/kushima-pg/tad04212026/06_wgs_primary_L2/output_v3"
)
WGS_L2_RESULTS = WGS_PRIMARY_OUT_DIR / "B_prime_L2_classes_results_v3.tsv"
WGS_SPEC_RESULTS = WGS_PRIMARY_OUT_DIR / "B_prime_specificity_groups_results_v3.tsv"

# v4 CHANGE: OUT_DIR を tad04212026/10_replication_2way_meta/output_v4 へ変更
OUT_DIR = Path(
...
def load_wgs_primary_results(l2_path: Path, spec_path: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Load tad04212026 06_wgs_primary_L2 output_v3 (R script 39_fit_B_prime_L2_and_specificity_v3)
    outputs and filter to comparison = ASD_vs_HC, sv_type = DEL, which matches the reference
    ASD-related arm used in replication (arrayCGH ASD_vs_CONT /
    MSSNG ASD_vs_unaffSib).
    """
...
def build_meta3way(
    ...
    key_cols must uniquely identify a test within each cohort result table.
    WGS is already pre-filtered (ASD_vs_HC, DEL) upstream.
    arrayCGH filter: ASD_vs_CONT ; MSSNG filter: ASD_vs_unaffSib.
    """
...
def main() -> None:
    t0 = time.time()
    log("=" * 60)
    log("Start 40_run_replication_full_pipeline_v4.py")
    ...
    log(f"  wgs_L2_results:   {WGS_L2_RESULTS}   [v4: from tad04212026 v3]")
    log(f"  wgs_spec_results: {WGS_SPEC_RESULTS} [v4: from tad04212026 v3]")
    log(f"  outdir:           {OUT_DIR}         [v4: tad04212026/10_replication_2way_meta/output_v4]")
    ...
    out_acgh = OUT_DIR / "arraycgh_burden_L2_and_specificity_v4.tsv"
    ...
    out_mssng = OUT_DIR / "mssng_burden_L2_and_specificity_v4.tsv"
    ...
    for qc in (acgh_qc, mssng_qc):
        for k, v in qc.items():
            if isinstance(v, dict):
                ...
                rows.append({"dataset": qc["dataset"],
                             "metric": k, "value": v})
    summary_path = OUT_DIR / "replication_burden_summary_v4.tsv"
    ...
    acgh_l2.to_csv(OUT_DIR / "arraycgh_L2_classes_results_v4.tsv",
                   sep="\t", index=False)
    mssng_l2.to_csv(OUT_DIR / "mssng_L2_classes_results_v4.tsv",
                    sep="\t", index=False)
    ...
    acgh_grp.to_csv(OUT_DIR / "arraycgh_specificity_groups_results_v4.tsv",
                    sep="\t", index=False)
    mssng_grp.to_csv(OUT_DIR / "mssng_specificity_groups_results_v4.tsv",
                     sep="\t", index=False)
    ...
    meta_l2.to_csv(OUT_DIR / "meta_L2_classes_results_v4.tsv",
                   sep="\t", index=False)
    ...
    meta_grp.to_csv(OUT_DIR / "meta_specificity_groups_results_v4.tsv",
                    sep="\t", index=False)
    ...
    # ================================================================
    log("#" * 60)
    log("Step D: 3-way IVW meta (WGS v3 ASD_vs_HC + arrayCGH v22 ASD_vs_CONT + MSSNG ASD_vs_unaffSib, DEL)")
    log("#" * 60)
    ...
    if not wgs_l2.empty:
        ...
        meta3_l2.to_csv(OUT_DIR / "meta3way_L2_classes_results_v4.tsv",
                        sep="\t", index=False)
    ...
    if not wgs_sp.empty:
        ...
        meta3_grp.to_csv(OUT_DIR / "meta3way_specificity_groups_results_v4.tsv",
                         sep="\t", index=False)
    ...
    summary_fit.to_csv(OUT_DIR / "replication_fit_summary_v4.tsv",
                       sep="\t", index=False)
    ...
    if not meta3_grp.empty:
        log("Preview: META3way group_primary carrier (WGS v3 + arrayCGH v22 + MSSNG IVW)")
