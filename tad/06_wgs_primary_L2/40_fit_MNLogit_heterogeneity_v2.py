...
# ファイル名: 40_fit_MNLogit_heterogeneity_v2.py
# 処理内容 (v1 -> v2 修正点):
...
#     (v1 と同一、上流変更なし)
#   - 出力: output_MNLogit_v2/
#       MNLogit_heterogeneity_results_v2.tsv  (20 individual + 4 global = 24 行)
#       MNLogit_heterogeneity_summary_v2.tsv  (sv 別 global 集計)
#       MNLogit_heterogeneity_qc_v2.json      (実行サマリ + 設定 + failed class list)
#   - 統計ロジックの本質 (10 L2 × 2 SV = 20 individual fit + Stouffer/Sign × 2)
#     は v1 と同じ。ただし v1 は sex_col バグで実行不能だったため、v2 が
#     事実上の初版となる。
#   - 実行時間記録あり
...
_OUTDIR = os.environ.get(
    "OUTDIR",
    # v2: sbatch の log 出力先と揃えるため output_v4 配下に変更
    # (v1 では output_MNLogit_v1 が SCRIPT_DIR 直下、logs が output_v4/output_MNLogit_v1 に
    #  分離されていた不整合を解消)
    str(OUT_06_B_PRIME_L2 / "output_v4" / "output_MNLogit_v2")
)
...
# Output filenames (v2 suffix)
_RESULTS_TSV = "MNLogit_heterogeneity_results_v2.tsv"
_SUMMARY_TSV = "MNLogit_heterogeneity_summary_v2.tsv"
_QC_JSON = "MNLogit_heterogeneity_qc_v2.json"
...
def main() -> int:
    t0 = time.time()
    log("=" * 60)
    log("40_fit_MNLogit_heterogeneity_v2.py")
    ...
    qc = {
        "script": "40_fit_MNLogit_heterogeneity_v2.py",
        "version": "v2",
        "v2_changes": [
