#!/usr/bin/env Rscript
# =============================================================================
# 39_fit_B_prime_L2_and_specificity_v7.R
# -----------------------------------------------------------------------------
# 処理内容 (v6 -> v7):
#   - [v7 主変更] S2-S5 sensitivity grouping を削除 (paper 不使用):
#     * GROUP_COLUMNS: c("group_primary", "group_s2", "group_s3", "group_s4", "group_s5")
#       -> c("group_primary")
#     * 解析 (ii) の出力 B_prime_specificity_groups_results_v7_{PATTERN}.tsv は
#       group_primary (S1) のみに縮小 (Diff_specific_n1, Diff_shared_n2plus,
#       Static の per-label B' fit)。これは Module 11 (Diff_any union) と Module
#       10 (replication meta) の中間 input として keep。
#     * 解析 (i) は変更なし (10 L2 classes B' fit、paper Fig 3a)。
#     * 統計 model (B' covariate, FDR within stratum) は v6 と完全一致。
#       group_primary に該当する数値結果は v6 と bit-identical。
#
#   - [path update] Module 05 v6 (S2-S5 削除済 burden) を input に切替:
#     * 入力: 05_wgs_sample_burden/output_v6_{PATTERN}/
#               sample_burden_L2_and_specificity_v6_{PATTERN}.tsv
#     * 出力: 06_wgs_primary_L2/output_v7_{PATTERN}/
#               B_prime_L2_classes_results_v7_{PATTERN}.tsv
#               B_prime_specificity_groups_results_v7_{PATTERN}.tsv
#               B_prime_run_summary_v7_{PATTERN}.tsv
#
...
#   Rscript 39_fit_B_prime_L2_and_specificity_v7.R \
#     [--burden <sample_burden_L2_and_specificity_v6_patX.tsv>] \
#     [--outdir <outdir>] \
#     [--min-cell-count 5]
# 出力:
#   B_prime_L2_classes_results_v7_{PATTERN}.tsv         (解析 i: paper Fig 3a)
#   B_prime_specificity_groups_results_v7_{PATTERN}.tsv (解析 ii: group_primary のみ)
#   B_prime_run_summary_v7_{PATTERN}.tsv                (fit 成否サマリ)
# =============================================================================
...
# -------------------------------------------------------------
# v7 (2026-04-29): PATTERN env var で patB / patC を切替
# DEFAULT_BURDEN / DEFAULT_OUTDIR は v7_{PATTERN} 用にハードコード override
# Module 05 v6 (S2-S5 削除済) 出力を入力とする
# -------------------------------------------------------------
PATTERN <- Sys.getenv("PATTERN", unset = "")
if (!(PATTERN %in% c("patB", "patC"))) {
  stop(sprintf(
    "v7 requires PATTERN env var set to 'patB' or 'patC'. Got: '%s'",
    PATTERN))
}
V7_SUFFIX <- sprintf("v7_%s", PATTERN)
...
# v7: Module 05 v6 burden -> Module 06 v7 outdir
DEFAULT_BURDEN <- file.path(PIPELINE_ROOT,
                            "05_wgs_sample_burden",
                            sprintf("output_v6_%s", PATTERN),
                            sprintf("sample_burden_L2_and_specificity_v6_%s.tsv", PATTERN))
DEFAULT_OUTDIR <- file.path(PIPELINE_ROOT,
                            "06_wgs_primary_L2",
                            sprintf("output_v7_%s", PATTERN))
...
# v7 変更: GROUP_COLUMNS から S2-S5 を削除 (paper 不使用)
...
t0 <- Sys.time()
log_msg(strrep("=", 60))
log_msg(sprintf("Start 39_fit_B_prime_L2_and_specificity_v7.R (PATTERN=%s) [S2-S5 removed]",
                PATTERN))
...
out_l2 <- file.path(outdir,
                    sprintf("B_prime_L2_classes_results_%s.tsv", V7_SUFFIX))
log_msg(sprintf("Writing: %s", out_l2))
write_tsv(l2_tbl, out_l2)

# -------------------------------------------------------------
# 解析 (ii): group_primary のみ (S2-S5 削除済 v7)
...
out_grp <- file.path(outdir,
                     sprintf("B_prime_specificity_groups_results_%s.tsv", V7_SUFFIX))
...
out_sum <- file.path(outdir,
                     sprintf("B_prime_run_summary_%s.tsv", V7_SUFFIX))
