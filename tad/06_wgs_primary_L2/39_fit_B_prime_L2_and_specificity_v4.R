#!/usr/bin/env Rscript
# =============================================================================
# 39_fit_B_prime_L2_and_specificity_v4.R
# -----------------------------------------------------------------------------
# 処理内容 (v3 -> v4):
#   - [path update のみ] 入力を 05_v3 (sample-level top-1% QC 適用済み burden)
#     に切り替え、出力を 06_v4/ に書き換える
#     * manifest JSON (paths_v1_manifest.json) は参照するが、
#       DEFAULT_BURDEN / DEFAULT_OUTDIR は v4 用にハードコード override する
#       (paths_v1.py を修正せずに v4 経路で動かすための措置)
#     * L2_CLASSES の整合性は引き続き manifest と突き合わせて検証
#   - 統計モデル・ロジックは v3 と完全一致
#   - 入力: 05_v3/sample_burden_L2_and_specificity_v3.tsv
#   - 出力: 06_wgs_primary_L2/output_v4/B_prime_*_v4.tsv
# 処理内容 (v3 既存):
#   - 38/v3 で作成した sample_burden_L2_and_specificity_v3.tsv を入力に
#     v200 Fig.3a / Supp Table 5 Panel A (10 L2 classes) と
#     specificity-group factorial の B' logistic regression を fit する
#   - 解析 (i): 10 L2 classes (HPC_Astro 除外)
#   - 解析 (ii): 5 group definitions (group_primary / s2 / s3 / s4 / s5)
...
#            + log1p_total_{del|dup}_bases
#            + log1p_total_gene_{DEL|DUP}
#   - 実行時間記録あり
# 使い方:
#   Rscript 39_fit_B_prime_L2_and_specificity_v4.R \
#     [--burden <sample_burden_L2_and_specificity_v3.tsv>] \
#     [--outdir <outdir>] \
#     [--min-cell-count 5]
# 出力:
#   B_prime_L2_classes_results_v4.tsv        (解析 i)
#   B_prime_specificity_groups_results_v4.tsv (解析 ii)
#   B_prime_run_summary_v4.tsv               (fit 成否サマリ)
# =============================================================================
...
# -------------------------------------------------------------
# v4 (2026-04-22): read paths_v1_manifest.json for L2_CLASSES 整合性 check のみ
# DEFAULT_BURDEN / DEFAULT_OUTDIR は v4 用にハードコード override する
...
# v4 hardcoded override (point at 05_v3 burden + 06_v4 outdir)
...
t0 <- Sys.time()
log_msg(strrep("=", 60))
log_msg("Start 39_fit_B_prime_L2_and_specificity_v4.R")
...
out_l2 <- file.path(outdir, "B_prime_L2_classes_results_v4.tsv")
...
out_grp <- file.path(outdir, "B_prime_specificity_groups_results_v4.tsv")
...
out_sum <- file.path(outdir, "B_prime_run_summary_v4.tsv")
