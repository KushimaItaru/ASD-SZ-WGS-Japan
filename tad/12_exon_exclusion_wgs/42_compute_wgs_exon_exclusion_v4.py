#!/usr/bin/env python3
# 42_compute_wgs_exon_exclusion_v4.py
#
# 処理内容:
#  - WGS discovery cohort (tad04292026 パイプライン) の exon-exclusion sensitivity
#    を計算する。
#  - [v3 -> v4 変更点]:
...
#      これらは Module 13 (MSSNG exon-exclusion) と完全に同一ロジックだったため、
#      集約してコード重複を削減 (~160 行削減)。helper-internal の数値結果は
...
#       - BOUNDARY_MASTER → F_01_BOUNDARY_MASTER
#       - EVENT_OVERLAP / BURDEN_TABLE → PIPELINE_ROOT 経由で構築
#    * 出力 dir / file: output_v3 → output_v4
#       - wgs_exon_exclusion_v3.tsv → wgs_exon_exclusion_v4.tsv
#       - wgs_exon_exclusion_summary_v3.tsv → wgs_exon_exclusion_summary_v4.tsv
#    * 解析ロジック (A1/A2/A3/A3-strict、Logit main + Firth sensitivity、
#      BH/Holm) は v3 と完全一致。
#    * group_primary / S2-S5 は使わない (Module 12/13 は exon-free 軸のみ;
#      Q1 の S2-S5 削除は Module 02/05/06 に閉じる)。
#  - [v2 -> v3 変更点 (継承)]: sample-level top-1% CNV count QC (Script 04 v10) を
#    反映した経路に切り替え
#    * event overlap : output_v9 -> output_v10 (v10 sample-level QC 適用済み)
#    * burden TSV    : output_v2 -> output_v3 (Script 05 v3 の出力、04_v10 由来)
#  - Input: tad04292026 v9/v10/v3 の処理済みファイル
#      01_heffel_boundary_master/output_v9/heffel_boundary_master_v9.tsv.gz
#      02_bin_l2_annotation/output_v3/bin_l2_annotation_v3.tsv.gz
#          (10 L2 class membership + bin 座標; hpc_astro 元から除外、S2-S5 削除済)
#      04_wgs_sv_boundary_overlap/output_v10/sample_boundary_event_overlap_v10.tsv.gz
#      05_wgs_sample_burden/output_v4/sample_burden_L2_and_specificity_v4.tsv
#          (v4: 04_v10 由来の共変量、S2-S5 削除済)
...
#           A3 軸 (SV レベル)   : exon 非重複 CNV かどうか
#           A3-strict            : exonfreeCNV かつ hit bin が exon-free
#      4. burden TSV に 5 exposure × 2 SVT 列を追加し、回帰:
#           Main: statsmodels.Logit
#           Sensitivity: 自前 Firth (QR rank reduction)
#      5. 解析グリッド: 2 comparisons × 2 SVT × 4 analyses
#      6. Multiple testing: ASD × DEL discovery-positive 4 行に BH + Holm
#      7. 出力:
#           wgs_exon_exclusion_v4.tsv
#           wgs_exon_exclusion_summary_v4.tsv
#  - 出力先: ${PIPELINE_ROOT}/12_exon_exclusion_wgs/output_v4/
#  - 処理時間を先頭と末尾で記録
...
#SBATCH --output=42_compute_wgs_exon_exclusion_v4_%j.out
#SBATCH --error=42_compute_wgs_exon_exclusion_v4_%j.err
...
# =========================================================
BOUNDARY_MASTER = (
    PIPELINE_ROOT / "01_heffel_boundary_master" / "output_v9"
    / "heffel_boundary_master_v9.tsv.gz"
)
BIN_L2_ANNOT = F_02_BIN_L2_ANNOTATION  # bin_l2_annotation_v3.tsv.gz (S2-S5 削除済)

# v4: 04_v10 の event overlap (Pattern A primary)
EVENT_OVERLAP = (
    PIPELINE_ROOT / "04_wgs_sv_boundary_overlap" / "output_v10"
    / "sample_boundary_event_overlap_v10.tsv.gz"
)
# v4: 05_v4 の burden (Pattern A primary, S2-S5 削除済)
BURDEN_TABLE = (
    PIPELINE_ROOT / "05_wgs_sample_burden" / "output_v4"
    / "sample_burden_L2_and_specificity_v4.tsv"
)

OUT_DIR = PIPELINE_ROOT / "12_exon_exclusion_wgs" / "output_v4"
OUT_TSV = OUT_DIR / "wgs_exon_exclusion_v4.tsv"
OUT_SUMMARY = OUT_DIR / "wgs_exon_exclusion_summary_v4.tsv"


# =========================================================
# CONSTANTS (Module 12-specific; L2_MEMBERSHIP_COLS は helper から)
# =========================================================
SVT_LIST = ["DEL", "DUP"]
COMPARISONS = [
...
# 4 discovery-positive (ASD × DEL only) for BH/Holm
...
# (Module 12 specific — keep in this script)
# =========================================================
...
# =========================================================
# STEP 4: QR rank reduction + Firth (Script 11 v4 porting)
# (Module 12 specific — kept as-is from v3)
# =========================================================
...
# =========================================================
# STEP 5: Standard Logit fit + carrier 集計 (Module 12 specific)
# =========================================================
...
def fit_logit_wgs(
    ...
    for xc in x_cols:
        ...
        results.append({"_xc": xc, "_cc": cc, "_cn": cn})
...
def fit_firth_wgs(
    ...
    protected = tuple(range(1 + len(x_cols)))
...
def main() -> None:
    log("=" * 72)
    log("Script 12 v4 START: WGS exon-exclusion sensitivity (tad04292026)")
    log("  42_compute_wgs_exon_exclusion_v4.py")
    log("  Helpers: common.exon_helpers (build_merged_exon_bed, load_bin_l2_annotation, etc.)")
    log("  Input: 04_v10 event overlap + 05_v4 burden (S2-S5 削除済)")
    log("=" * 72)
    ...
    elapsed_total = time.time()  # 注: 実 elapsed は helper の log 内で表示済 (helper の _T0 から計測)
    log("\n" + "=" * 72)
    log(
        f"Script 12 v4 DONE: 42_compute_wgs_exon_exclusion_v4.py"
    )
    log("=" * 72)
