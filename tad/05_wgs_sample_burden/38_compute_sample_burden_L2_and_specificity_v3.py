...
# -*- coding: utf-8 -*-
"""
38_compute_sample_burden_L2_and_specificity_v3.py
=====================================================
処理内容:
  - [v3 変更点] 入力を Script 04 v10 (sample-level top-1% CNV count QC 適用済み)
    のファイルに切り替え、出力は output_v3/ に出す。
    ...
    * 出力: output_v3/sample_burden_L2_and_specificity_v3.tsv(.summary)
    * ロジックは v2 と同一 (path update のみ)。
...
入力:
  ...
  --outdir          : 出力ディレクトリ (output_v3/)
出力:
  sample_burden_L2_and_specificity_v3.tsv
  sample_burden_L2_and_specificity_summary_v3.tsv (QC)
"""
...
# =========================================================
# v3: override Step 04 inputs to point at v10 (sample-level QC 適用済み)
# =========================================================
...
# v3 output dir
OUT_05_V3 = PIPELINE_ROOT / "05_wgs_sample_burden" / "output_v3"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Compute sample x (L2 classes + specificity groups) x SV type burden (v3: input=04_v10)."
    )
    ...
    p.add_argument("--outdir", default=OUT_05_V3, type=Path,
                   help="Output directory (output_v3/, will be created).")
...
# and consumed as hard requirements by Step 6 (39_fit_B_prime_L2_and_specificity_v4.R).
...
def main() -> None:
    ...
    t0 = time.time()
    log("=" * 60)
    log("Start 38_compute_sample_burden_L2_and_specificity_v3.py")
    ...
    #        NOTE: 04_v10 burden の sample universe で右側に寄せる。burden にのみ含まれる
    ...
    # ---- 6. Write ----
    out_final = args.outdir / "sample_burden_L2_and_specificity_v3.tsv"
    log(f"Writing: {out_final}")
    final.to_csv(out_final, sep="\t", index=False)

    summary = make_summary(final)
    out_sum = args.outdir / "sample_burden_L2_and_specificity_summary_v3.tsv"
