...
echo "  1) python3 ehdn/01_prepare_sample_lists_v2.py"
echo "  2) strling/01_calc_depth_array_fast_v2.sh  (array job)"
...
echo "           sed \"s/ARRAY_SIZE/\${N}/\" strling/01_calc_depth_array_fast_v2.sh | sbatch"
echo "  3) python3 strling/03_collect_depths_v1.py   (after depth jobs finish)"
echo "  4) ehdn/02_run_ehdn_array_v2.sh  (array job)"
echo "     NOTE: Same ARRAY_SIZE replacement needed."
echo "           sed \"s/ARRAY_SIZE/\${N}/\" ehdn/02_run_ehdn_array_v2.sh | sbatch"
echo "  5) python3 ehdn/04_merge_ehdn_novel_norm_v2.py  (after EHdn jobs finish)"
echo "  6) python3 ehdn/14_outlier_burden_rare_casecontrol_crossfit_v19.py"
echo "  7) python3 ehdn/17_burden_statistical_test_v20.py"
echo ""
echo "Or use the wrapper scripts in str_03242026/ (recommended)."
